...
import { WeaveMemoryTool } from "./weave_memory"
import { WeaveIdeaTool } from "./weave_idea"
import { WeaveEventTool } from "./weave_event"
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const weaveMemory = yield* WeaveMemoryTool
    const weaveIdea = yield* WeaveIdeaTool
    const weaveEvent = yield* WeaveEventTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          plan: Tool.init(plan),
          weave_memory: Tool.init(weaveMemory),
          weave_idea: Tool.init(weaveIdea),
          weave_event: Tool.init(weaveEvent),
        })
...
export const defaultLayer = Layer.suspend(() =>
  layer
    .pipe(
      ...
      Layer.provide(WeaveMemory.defaultLayer),
      Layer.provide(WeaveIdea.defaultLayer),
    )
